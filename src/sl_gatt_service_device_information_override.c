...
#if defined(gattdb_firmware_revision_string) && defined(gattdb_firmware_revision_string_len) && \
    defined(FIRMWARE_REVISION_STRING)
...
static_assert(gattdb_firmware_revision_string_len >= FIRMWARE_REVISION_STRING_LEN,
              "Bluetooth stack version does not fit into firmware revision string characteristic. Please adjust GATT "
              "configuration.");
...
#if defined(gattdb_hardware_revision_string) && defined(gattdb_hardware_revision_string_len) && \
    defined(HARDWARE_REVISION_STRING)
#define HARDWARE_REVISION_STRING_LEN (sizeof(HARDWARE_REVISION_STRING) - 1)
static_assert(
    gattdb_hardware_revision_string_len >= HARDWARE_REVISION_STRING_LEN,
...
#if defined(gattdb_firmware_revision_string) && defined(gattdb_firmware_revision_string_len) && \
    defined(FIRMWARE_REVISION_STRING)
            sc = sl_bt_gatt_server_write_attribute_value(gattdb_firmware_revision_string, 0,
                                                         sizeof(firmware_revision_string), firmware_revision_string);
...
// Configurator.
            ...
            // Model Number String
#if defined(gattdb_model_number_string) && defined(gattdb_model_number_string_len) && defined(MODEL_NUMBER_STRING)
            sc = sl_bt_gatt_server_write_attribute_value(gattdb_model_number_string, 0, sizeof(model_number_string),
                                                         model_number_string);
            app_assert_status(sc);
#else
...
// GATT Configurator to set the value manually.
    ...
    defined(HARDWARE_REVISION_STRING)
            sc = sl_bt_gatt_server_write_attribute_value(gattdb_hardware_revision_string, 0,
                                                         sizeof(hardware_revision_string), hardware_revision_string);
            app_assert_status(sc);
#else
...
// GATT Configurator to set the value manually.
            ...
            sc = sl_bt_gatt_server_write_attribute_value(gattdb_system_id, 0, gattdb_system_id_len, system_id);
            app_assert_status(sc);
#else
