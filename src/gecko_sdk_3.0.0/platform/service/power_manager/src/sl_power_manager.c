...
sl_status_t sl_power_manager_init(void)
{
  ...
  if (!is_initialized) {
    ...
    // Set the default wake-up overhead value and the HF minimum offtime in sleeptimer ticks
...
/***************************************************************************//**
 * Use sleeptimer to calculate the time between two tick counts.
 ******************************************************************************/
void sli_power_manager_calculate_wakeup_time(uint32_t *time)
{
  uint32_t tick_count;
  CORE_DECLARE_IRQ_STATE;

  // Get current sleeptimer tick count
  tick_count = sl_sleeptimer_get_tick_count();

  // Calculate time between the start and end of a wake-up process
  CORE_ENTER_CRITICAL();
  *time = (tick_count - wakeup_start_tick_count);
  CORE_EXIT_CRITICAL();
}
